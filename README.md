...
Documentation on how to use the bridge can be found in the C code that uses it, but I'll give a concise version here, too. The ARM can write to the storage of the two arrays with 4 int8 numbers at a time. Assuming both arrays are initialized row-major, B can be stored directly but A *must* be stored transposed. The final C array will contain 64 int32 numbers, which can be read one at a time. All 64 MACs in the array receive a row (from B) and a column (from A) every broadcast cycle, so counting one startup cycle the full process takes 9 clock cycles for an 8x8 array. Here's a basic word-addressed memory map:
...
| `Python/golden_model.py` | Python script to generate a golden model of matrix multiplication to test against. WRITTEN BY CLAUDE, NOT BY ME! |
...
- Mid September: Converted to a systolic array
- Late September: Full interface completed and published
- Mid November: Final bug fixes and improvements, then this project is done!

I'll also be doing a full writeup about my experience building this project. When that's done, you'll be able to find it here: https://ae0lis.github.io/portfolio/.
